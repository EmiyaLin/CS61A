...
def digit_distance(n):
    ...
    def distace(n):
        if n < 10:
            return 0
        return abs((n // 10) % 10 - n % 10)

    if n < 10:
        return 0
    return distace(n) + digit_distance(n // 10)
...
def count_coins(total):
    ...
    def count_helper(total, coin_value):
        if total == 0:
            return 1
        if total < 0 or coin_value == None:
            return 0
        with_current_coin = count_helper(total - coin_value, coin_value)
        without_current_coin = count_helper(total, next_smaller_coin(coin_value))
