...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        grid = self.place
        distance = 0
        while not grid.is_hive:
            if grid.bees and self.lower_bound <= distance <= self.upper_bound :
                return random_bee(grid.bees)
            grid = grid.entrance
            distance += 1
        return None
        # END Problem 3 and 4
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.turns_to_chew:
            ...
            if cur_place.bees:
                chewed_bee = random_bee(cur_place.bees)
...
class TankAnt(ContainerAnt):
    """"TankAnt provides protection to other Ants and dose 1 damage to bees in the place"""
    name = 'Tank'
    implemented = True
    food_cost = 6
    damage = 1
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        grid = self.place.exit
        while grid:
            if grid.ant:
                grid.ant.double()
                if grid.ant.is_container and grid.ant.ant_contained:
                    grid.ant.ant_contained.double()
            grid = grid.exit
        # END Problem 12
