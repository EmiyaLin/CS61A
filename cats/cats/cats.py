...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    picked_paragraphs = [paragraph for paragraph in paragraphs if select(paragraph)]
    if len(picked_paragraphs) <= k:
        return ''
    else:
...
def feline_fixes(typed, source, limit):
    ...
    def helper(typed, source, count):
        ...
        if count > limit:
            return count
        return helper(typed[1:], source[1:], count)

    count = helper(typed, source, 0)
    print("DEBUG:", count)
    return count
